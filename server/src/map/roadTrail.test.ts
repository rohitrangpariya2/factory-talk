...
describe('road trail script', () => {
  test('renders OSRM map matching helpers with segmented matched geometry', () => {
    const script = buildRoadTrailScript();

    expect(script).toContain('/road-match');
    expect(script).toContain('ROAD_MATCH_MAX_POINTS = 60');
    expect(script).toContain('ROAD_MATCH_MIN_DISTANCE_METERS = 15');
    ...
    expect(script).toContain('ROAD_MATCH_MIN_CONFIDENCE = 0.35');
    ...
    expect(script).toContain("throw new Error('Road match fallback: ' + reason)");
    expect(script).toContain('roadLatLngs.roadSegments = roadSegments');
    expect(script).toContain('roadLatLngs.roadMatchConfidence');
    expect(script).toContain('roadLatLngs.unmatchedGapsCount');
